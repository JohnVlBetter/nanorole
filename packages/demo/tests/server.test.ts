...
describe("demo server", () => {
  ...
  test("proxies story state endpoint to Python core", async () => {
    const seen: string[] = [];
    const runtime = createServer(async (request: IncomingMessage, response: ServerResponse) => {
      seen.push(`${request.method} ${request.url}`);
      response.setHeader("content-type", "application/json");
      response.end(JSON.stringify({ currentScene: "The observatory clock has stopped..." }));
    });
    const runtimeUrl = await listen(runtime);
    const demo = createDemoServer({ projectRoot: process.cwd(), runtimeUrl });
    const baseUrl = await listen(demo);

    await expect(fetch(`${baseUrl}/api/sessions/s1/story-state`).then((response) => response.json())).resolves.toEqual({
      currentScene: "The observatory clock has stopped..."
    });

    expect(seen).toEqual(["GET /v1/sessions/s1/story-state"]);
  });
